...
public class Card : MonoBehaviour
{
    ...
    private Quaternion _fliptRot = Quaternion.Euler(0, 0, 180);
    ...
    private void AddDecorators()
    {
        ...
        foreach (JsonPip pip in JsonParseDeck.DECORATORS)
        {
            ...
            if (pip.flip) _tGO.transform.rotation = _fliptRot;
            ...
            _tGO.name = pip.type;
            // Addthis decorator GameObject to the List card.decoGOs
            decoGOs.Add(_tGO);
        }
    }
    ...
    private void AddPips()
    {
        ...
        foreach(JsonPip pip in def.pips)
        {
            ...
            if (pip.flip) _tGO.transform.rotation = _fliptRot;
            // Sclae it if necessary (only for the Ace)
            if (pip.scale != 1)
            {
                _tGO.transform.localScale = Vector3.one * pip.scale;
            }
            ...
            _tSRend.sortingOrder = 1;
            // Add this to the Card'slist of pips
            pipGOs.Add(_tGO);
        }
    }
